...
namespace ZXing.Test
{
    ...
    public class RGBLuminanceSourceTests
    {
        ...
        public void Test()
        {
            // Crash 1 - IOOB in MaxiCodeReader.extractPureBits (79 bytes)
            var hexStr = "2000029393939393939393939393939361921b1a38e2dc1f99ea90fcefaa7a0e6dc8c21d9bc8eadc600406674f9e4ae091a033b8e99bff7f648acc893cba08ff7fffff86b71b196c89ab008000002f";
            var crash1 = Enumerable.Range(0, hexStr.Length / 2).Select(x => Convert.ToByte(hexStr.Substring(x * 2, 2), 16)).ToArray();

            int width = (crash1[0] % 64) + 1;
            int height = (crash1[1] % 64) + 1;
            var luminance = new byte[width * height];
            //crash1.AsSpan(2, Math.Min(crash1.Length - 2, luminance.Length)).CopyTo(luminance);
            Array.Copy(crash1, 2, luminance, 0, luminance.Length);
            //crash1.CopyTo(luminance, 2);
            var source = new RGBLuminanceSource(luminance, width, height);
            var bitmap = new BinaryBitmap(new HybridBinarizer(source));
            new MultiFormatReader().decode(bitmap);
        }

        [TestCase(RGBLuminanceSource.BitmapFormat.Gray8)]
        [TestCase(RGBLuminanceSource.BitmapFormat.Gray16)]
        [TestCase(RGBLuminanceSource.BitmapFormat.UYVY)]
        [TestCase(RGBLuminanceSource.BitmapFormat.ARGB32)]
        [TestCase(RGBLuminanceSource.BitmapFormat.BGR24)]
        [TestCase(RGBLuminanceSource.BitmapFormat.BGR32)]
        [TestCase(RGBLuminanceSource.BitmapFormat.BGRA32)]
        [TestCase(RGBLuminanceSource.BitmapFormat.RGB24)]
        [TestCase(RGBLuminanceSource.BitmapFormat.RGB32)]
        [TestCase(RGBLuminanceSource.BitmapFormat.RGB565)]
        [TestCase(RGBLuminanceSource.BitmapFormat.RGBA32)]
        public void Should_Not_Crash_If_Data_Is_To_Small(RGBLuminanceSource.BitmapFormat format)
        {
            // Crash 2 - IOOB in RGBLuminanceSource.CalculateLuminanceRGB24
            // Root cause: width*height*3 > rgbRawBytes.Length
            // Minimal repro:
            var data = new byte[10];
            var source = new RGBLuminanceSource(data, 100, 100, format); // expects 30000 bytes, gets 10
            data = new byte[11];
            source = new RGBLuminanceSource(data, 100, 100, format); // expects 30000 bytes, gets 11
            data = new byte[12];
            source = new RGBLuminanceSource(data, 100, 100, format); // expects 30000 bytes, gets 12
        }
    }
}
