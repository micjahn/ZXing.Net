  ...
  private bool[] extractBits(BitMatrix matrix) 
  {
    ...
    while (layer != 0) {
      ...
      for (int i = 0; i < 2*size - 4; i++) {
        rawbits[rawbitsOffset+i] = matrix.get_Renamed(matrixOffset + flip, matrixOffset + i/2);
        rawbits[rawbitsOffset+2*size - 4 + i] = matrix.get_Renamed(matrixOffset + i/2, matrixOffset + size-1-flip);
        flip = (flip + 1)%2;
      }

      flip = 0;
      for (int i = 2*size+1; i > 5; i--) {
        rawbits[rawbitsOffset+4*size - 8 + (2*size-i) + 1] = matrix.get_Renamed(matrixOffset + size-1-flip, matrixOffset + i/2 - 1);
        rawbits[rawbitsOffset+6*size - 12 + (2*size-i) + 1] = matrix.get_Renamed(matrixOffset + i/2 - 1, matrixOffset + flip);
        flip = (flip + 1)%2;
      }
  ...
  private static BitMatrix removeDashedLines(BitMatrix matrix) {
    ...
    for (int x = 0; x < matrix.Width; x++) {
      ...
      for (int y = 0; y < matrix.Height; y++) {
        ...
        if (matrix.get_Renamed(x, y)) {
          newMatrix.set_Renamed(nx, ny);
        }
        ny++;
      }
      nx++;
