...
namespace ZXing.Datamatrix.Encoder
{
    ...
    /// @author Alex Geller
    /// </summary>
    internal static class MinimalEncoder
    {
        internal enum Mode
        {
            ASCII,
        ...
        static char[] C40_SHIFT2_CHARS = {'!', '"', '#', '$', '%', '&', '\'', '(', ')', '*',  '+', ',', '-', '.', '/',
                                          ':', ';', '<', '=', '>', '?',  '@', '[', '\\', ']', '^', '_' };

        static bool isExtendedASCII(char ch, int fnc1)
        {
        ...
        public static String encodeHighLevel(String msg, Encoding priorityCharset, int fnc1, SymbolShapeHint shape)
        {
            ...
            {
                macroId = 5;
                msg = msg.Substring(HighLevelEncoder.MACRO_05_HEADER.Length, msg.Length - HighLevelEncoder.MACRO_05_HEADER.Length);
            }
            else if (msg.StartsWith(HighLevelEncoder.MACRO_06_HEADER) && msg.EndsWith(HighLevelEncoder.MACRO_TRAILER))
            {
                macroId = 6;
                msg = msg.Substring(HighLevelEncoder.MACRO_06_HEADER.Length, msg.Length - HighLevelEncoder.MACRO_06_HEADER.Length);
            }
            var bytes = encode(msg, priorityCharset, fnc1, shape, macroId);
            return StringUtils.ISO88591_ENCODING.GetString(bytes, 0, bytes.Length);
        }
         ...
         */
        static byte[] encode(String input, Encoding priorityCharset, int fnc1, SymbolShapeHint shape, int macroId)
        {
            return encodeMinimally(new Input(input, priorityCharset, fnc1, shape, macroId)).Bytes;
        }

        static void addEdge(Edge[][] edges, Edge edge)
        {
            var modeInt = (int)(edge.getEndMode());
            int vertexIndex = edge.fromPosition + edge.characterLength;
            if (edges[vertexIndex][modeInt] == null ||
                edges[vertexIndex][modeInt].cachedTotalSize > edge.cachedTotalSize)
            {
                edges[vertexIndex][modeInt] = edge;
            }
        }
        ...
        static int getNumberOfC40Words(Input input, int from, bool c40, int[] characterLength)
        {
            ...
            for (int i = from; i < input.Length; i++)
            {
                ...
                if (thirdsCount % 3 == 0 || ((thirdsCount - 2) % 3 == 0 && i + 1 == input.Length))
                {
                    ...
                    return (int)Math.Ceiling(((double)thirdsCount) / 3.0);
                }
            }
            characterLength[0] = 0;
            return 0;
        ...
        internal class Edge
        {
            private static int[] allCodewordCapacities = {3, 5, 8, 10, 12, 16, 18, 22, 30, 32, 36, 44, 49, 62, 86, 114,
                                                        144, 174, 204, 280, 368, 456, 576, 696, 816, 1050, 1304, 1558};
            private static int[] squareCodewordCapacities = {3, 5, 8, 12, 18, 22, 30, 36, 44, 62, 86, 114, 144, 174, 204,
                                                           280, 368, 456, 576, 696, 816, 1050, 1304, 1558};
            private static int[] rectangularCodewordCapacities = { 5, 10, 16, 33, 32, 49 };
            internal Input input;
            private Mode mode; //the mode at the start of this edge.
            internal int fromPosition;
            internal int characterLength;
            internal Edge previous;
            internal int cachedTotalSize;

            internal Edge(Input input, Mode mode, int fromPosition, int characterLength, Edge previous)
            {
                this.input = input;
                this.mode = mode;
            ...
            internal Mode getPreviousStartMode()
            {
                return previous == null ? Mode.ASCII : previous.mode;
            }

            internal Mode getPreviousMode()
            {
            ...
            static int getShiftValue(char c, bool c40, int fnc1)
            {
            ...
            private static int getC40Value(bool c40, int setIndex, char c, int fnc1)
            {
                if (c == fnc1)
                {
                    //assert setIndex == 2;
            ...
            byte[] getC40Words(bool c40, int fnc1)
            {
                var c40Values = new List<byte>();
            ...
            byte[] getEDFBytes()
            {
                int numberOfThirds = (int)Math.Ceiling(characterLength / 4.0);
        ...
        internal class Result
        {
            ...
            public Result(Edge solution)
            {
                ...
                var randomizeLengths = new List<int>();
                ...
                while (current != null)
                {
                    ...
                    {
                        if (current.Mode == Mode.B256)
                        {
                            if (size <= 249)
                            {
                                bytesAL.Insert(0, (byte)size);
                                size++;
                            }
                            else
                            {
                                bytesAL.Insert(0, (byte)(size % 250));
                                bytesAL.Insert(0, (byte)(size / 250 + 249));
                                size += 2;
                            }
                ...
                for (int i = 0; i < randomizePostfixLength.Count; i++)
                {
            ...
            static void applyRandomPattern(List<Byte> bytesAL, int startPosition, int length)
            {
                for (int i = 0; i < length; i++)
                {
                    ...
                    bytesAL[Pad_codeword_position] = (byte)(temp_variable <= 255 ? temp_variable : temp_variable - 256);
                }
            }
