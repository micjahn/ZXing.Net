...
namespace ZXing.EmguCV.Test
{
    ...
    [TestFixture]
    public class ImageLuminanceSourceTests
    {
        ...
        {
            using (var bitmap = LoadEmguCVImage(fileName))
            {
                var luminance = new ImageLuminanceSource(bitmap);
        ...
        public void Luminance_Is_Within_Margin_Of_Error(string fileName)
        {
            ...
            {
                var bitmapLuminanceSource = new ZXing.Windows.Compatibility.BitmapLuminanceSource(control);
            ...
            using (var bitmap = LoadEmguCVImage(fileName))
            {
                var imagesharpBitmapLuminanceSource = new ImageLuminanceSource(bitmap);
            ...
            for (int index = 0; index < controlLuminanceSourceMatrix.Length; index++)
            {
                ...
                Assert.That(testColor, Is.EqualTo(controlColor).Within(22)); // would be better around 3
            }
        }

        private static Emgu.CV.Image<Emgu.CV.Structure.Bgr, byte> LoadEmguCVImage(string fileName)
        {
            var file = fileName;
            if (!Path.IsPathRooted(file))
                file = Path.GetFullPath(Path.Combine(TestContext.CurrentContext.TestDirectory, fileName));
            if (!File.Exists(file))
            {
            ...
            return new Emgu.CV.Image<Emgu.CV.Structure.Bgr, byte>(file);
        }
